""" This module contains the Favorite class, allowing to find and save the user's favorite products. """
import django
...
class Favorites:
    ...
    def statut_fav_from_prod(self, r_product, r_user):
        ...
        for loop in r_product:
            favorite = Favorite.objects.filter(id_user_id=r_user, id_product_id=loop.pk)
            if favorite:
                for loop1 in favorite:
                    list_prod.append({
                        'active': loop1.active, 'id_user': loop1.id_user_id, 'id_product_fav': loop1.id_product_id,
